...
# For reading tweet from the stream
def readTweets():
...
def DBcreate(time, rdd):
    ...
    if rdd.count() > 0:
        ...
        if (data.count() > 0):
            ...
            df.write.mode("append").saveAsTable("hashtag_table")
        else:
            print("No more hashtag")
        ...
        df.write.mode("append").saveAsTable("keyword_table")
        ...
        df.write.mode("append").saveAsTable("participants_table")

        # Creating table for ocurrences
        data = rdd.map(lambda x: x["extended_tweet"]["full_text"])
        data = data.map(lambda x: x.split(" ")).flatMap(lambda x: x).map(lambda x: x.upper())
        ...
        if data.count()>0:
            ...
            df.show()
            df.write.mode("append").saveAsTable("ocurrences_table")
        else:
            print("RDD is empty")

if __name__ == "__main__":
    sc = SparkContext(appName="JamelProject BD2: DBform")
    readTweets()
